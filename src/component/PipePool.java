...
public class PipePool {
    ...
    public static final int maxPipeCount = 30;//对象池中对象的最大个数，自行定义
    public static final int fullPipe = (Constant.frameWidth / (Pipe.pipeHeadWidth + GameElementLayer.horizontalInterval) + 2) *2;

    static {
        for (int i = 0; i < PipePool.fullPipe; i++) {
            pool.add(new Pipe());
        }
        for (int i = 0; i < PipePool.fullPipe; i++) {
    ...
    public static void giveBack(Pipe pipe) {
        ...
        if (pipe.getClass() == Pipe.class) {
            if (pool.size() < maxPipeCount) {
                pool.add(pipe);
            }
        } else {
            if (movingPool.size() < maxPipeCount) {
